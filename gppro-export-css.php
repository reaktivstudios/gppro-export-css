...
class GP_Pro_Export_CSS {
	...
	public function export_css_notices() {
		...
		if ( ! isset( $_GET['page'] ) || isset( $_GET['page'] ) && 'genesis-palette-pro' !== $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,
			return;
		}

		// Check our CSS export action.
		if ( ! isset( $_GET['export-css'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,
			return;
		}

		// Check for non failure.
		if ( isset( $_GET['export-css'] ) && 'failure' !== $_GET['export-css'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,
			return;
		}
	...
	public function export_css_file() {
		// Check nonce.
		if ( empty( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'gppro_css_export_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			return;
		}
		...
		echo $this->escape_css( $output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		exit();
	}
